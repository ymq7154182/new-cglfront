...
<script>
    export default {
        ...
        methods: {
            ...
            searchClass() {
                if(!this.currentSchoolYear || !this.currentPeriod || !this.currentClassName) {
                    this.$message.success("请选择学年，学期以及班级进行查询");
                    return;
                }else {
                    this.showStudentsName = false;
                    this.showClassStudentsName = true;
                    
                    var url = `http://58.119.112.12:11030/rest/getStudentSelectByClass?academicyear=${this.currentSchoolYear}&term=${this.currentPeriod}&teacherid=90807760&classname=${this.currentClassName}`;
            ...
            searchSubject() {
                if(!this.currentSchoolYear || !this.currentPeriod || !this.currentSubject) {
                    this.$message.success("请选择学年，学期以及科目进行查询");
                    return;
                }else {
                    this.showStudentsName = true;
                    this.showClassStudentsName = false;
                    var url = `http://58.119.112.12:11030/rest/getStudentSelectByCourse?academicyear=${this.currentSchoolYear}&term=${this.currentPeriod}&teacherid=90807760&classname=${this.currentSubject}`;
                    this.$axios.get(url).then((res) => {
